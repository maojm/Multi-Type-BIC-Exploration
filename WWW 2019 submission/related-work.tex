...
\asedit{Recommendation policies with no explicit exploration, and closely related ``greedy" (exploitation-only) algorithm in multi-armed bandits, have been a subject of a recent line of work \cite{bastani2017exploiting,Sven-aistats18,kannan2018smoothed,externalities-colt18}.
A common theme is that the greedy algorithm performs well in theory, under  substantial type diversity assumptions. Yet, it is well-known that it suffers $\Omega(T)$ regret in the worst case.}%
