...
A very recent paper \cite{Kempe-colt18} (published after we finalized our results) studies incentivizing exploration with heterogenous agents and private types, but allows monetary transfers. Assuming that each arm is preferred by some agent type, they design an algorithm with a (very) low regret, and conclude that \emph{diversity helps} in their setting.
