...
Bayesian Exploration with heterogenous agents and public types is studied in \cite{ICexploration-ec15}, under a very strong assumption which ensures explorability of all type-action pairs, and in \cite{ICexplorationGames-ec16}, where a fixed tuple of agent types arrives in each round and plays a game. \asedit{\cite{ICexplorationGames-ec16} focus on explorability of joint actions. Our approach for the public-type case is similar on a high level, but simpler and more efficient, essentially because we focus on type-action pairs rather than joint actions.}
