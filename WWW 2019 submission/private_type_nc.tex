...
\begin{proof}
\asedit{For each signal realization $s$, denote 
...
\ascomment{Jieming:  you said the next eqn is not quite right!}
\begin{align*}
...
\cdot  \Pr[\pi(s) = m] \\
&\quad= \sum_{\omega,s,s'} \Pr[\omega_0 = \omega, S = s] \cdot \Pr[ S'=s'|S= s] \cdot \Pr[\pi'(s') = m]   \\
&\qquad\qquad\cdot \left(u(\theta, m(\theta), \omega) - u(\theta, m'(\theta), \omega)\right)\\
&\quad\geq \sum_{\omega,s,s'} \Pr[\omega_0 = \omega, S = s, S'=s'] \cdot \Pr[\pi'(s') = m]   \\
...
Given a menu $m$, an action-reward pair will be revealed to the algorithm after the round. Assuming the agent is following the menu, such action-reward pair is called a sample of the menu $m$. \asdelete{We use $D_m$ to denote the distribution of the samples. $D_m$ is a random variable depending on the state $\omega_0$.} For a fixed state $\omega$, let $D_m(\omega)$ denote the distribution of the samples of menu $m$.
...
\asdelete{\begin{corollary}[Restatement of Theorem \ref{thm:private_nocc}]
