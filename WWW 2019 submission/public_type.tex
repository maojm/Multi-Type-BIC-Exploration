...
After that, for each agent type $\theta$, Algorithm \ref{alg:public_main} recommends the best explored action: \\$ \arg\max_{a \in \AExp_{\omega,\theta}} u(\theta, a, \omega)$ \jmedit{with probability exactly 1. You may wonder how we guarantee this probability to be 1 if some low-probability event happens (e.g. all agents have the same type). The answer is, in that case, our algorithm would never finish $|\A|\cdot |\varTheta|$ phases. But as long as the algorithm finishes $|\A|\cdot |\varTheta|$ phases, we have the above guarantee.} 
