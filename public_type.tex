...
%This section is very similar to the EC16's paper, we include this section to make our paper self-contained.
Let us design a subroutine, called  MaxExplore, which explores all signal-explorable actions with probability $1$. We are going to assume $L_{\theta} \geq \max_{a,s: \Pr[\pi^{max}(s)=a] \neq 0} \frac{1}{\Pr[\pi^{max}(s)=a]}$.
...
Given type $\theta$ and realized signal $S$, MaxExplore explores all actions that are signal-explorable for $\theta$ and $S$.
