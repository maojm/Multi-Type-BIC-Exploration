...
Appendix~\ref{app:info-theory} for background). Essentially, we show that a more informative signal leads to the same or larger explorable set.
...
&\textbf{subject to: }\\
    ...
    \Pr[\omega] \cdot \Pr[s \mid  \omega] \cdot\\
        &\left(u(\theta, a, \omega) - u(\theta, a', \omega)\right) \cdot x_{a,s} \geq 0   &\qquad & \forall a,a' \in \A \\
...
L_{\theta} \geq \max_{(a,s)\in \A\times \X \text{ with } \Pr[\pi^{\max}(s)=a] \neq 0} \quad \frac{1}{\Pr[\pi^{\max}(s)=a]}.
\end{align}
...
For details, see Algorithm \ref{alg:public_explore}.
 \begin{algorithm}[H]
	...
	%\IF {$l \leq |\A| \cdot |\varTheta|$}
		...
		\STATE Sample $Res$ many actions from distribution $\pRes$ independently and add these actions into $\alpha$.
 ...
 \begin{algorithm}[t]
	...
	\FOR {rounds $t=1$ to $T$}
        ...
        \STATE Let $\S_\ell$ be the signal structure for $S_\ell$
         given the realized type sequence $(\theta_1,...,\theta_t)$.
			\ENDIF
		\ELSE
