...
\jmcomment{
When types are public, we can always simulate a BIC policy for private types. This implies a connection between eventually-explorable actions and eventually-explorable menus: for all $\omega \in \varOmega, \theta \in \varTheta, m\in \MExp_{\omega}$, we have $m(\theta) \in \AExp_{\omega,\theta}$. Then we have $\OPTpub \geq \OPTpri$. Example \ref{exp:simple} shows that there are cases in which $\OPTpub$ is strictly larger than $\OPTpri$. 
}
