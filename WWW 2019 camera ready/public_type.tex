...
\footnote{\asedit{This holds with probability exactly 1, provided that our algorithm finishes $|\A|\cdot |\varTheta|$  phases. If some undesirable low-probability event happens, \eg if all agents seen so far have had the same type, our algorithm would never finish $|\A|\cdot |\varTheta|$ phases.}}
